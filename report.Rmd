...
CAR_random_forest <- client_analytical_record %>%
  ...
  select(-user_disp_id, -user_client_id, -user_district_id, -user_sex, 
         ...
         -card_id, -card_issued, 
         -contains("1998")) %>%
  mutate(get_loan = ifelse(account_id %in% loan$account_id | is.na(account_id), "yes", "no")) %>%
  mutate(card_type = coalesce(card_type, "noCard"))
...
# Setting up the train-test split
set.seed(123) # For reproducibility
splitIndex <- createDataPartition(CAR_random_forest$get_loan, p = .8, list = FALSE)
train_set <- CAR_random_forest[splitIndex, ]
test_set <- CAR_random_forest[-splitIndex, ]

new_model <- randomForest(CAR_random_forest ~ ., data = train_set, method = "rf")
```
