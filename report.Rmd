...
loan$status <- ifelse(loan$status == "A", "finished_ok", loan$status)
loan$status <- ifelse(loan$status == "B", "finished_not_payed", loan$status)
loan$status <- ifelse(loan$status == "C", "running_ok", loan$status)
loan$status <- ifelse(loan$status == "D", "running_client_in_dept", loan$status)
...
order$k_symbol <- ifelse(order$k_symbol == "POJISTNE", "insurrance payment", order$k_symbol)
order$k_symbol <- ifelse(order$k_symbol == "SIPO", "household", order$k_symbol)
order$k_symbol <- ifelse(order$k_symbol == "LEASING", "leasing", order$k_symbol)
order$k_symbol <- ifelse(order$k_symbol == "UVER", "loan payment", order$k_symbol)
order$k_symbol <- ifelse(order$k_symbol == " ", NA, order$k_symbol)
...
trans$type <- ifelse(trans$type == "PRIJEM", "credit", trans$type)
trans$type <- ifelse(trans$type == "VYDAJ", "withdrawal", trans$type)
trans$type <- ifelse(trans$type == "VYBER", "withdrawal in cash", trans$type)

# Operation Werte verständlich machen
trans$operation <- ifelse(trans$operation == "VYBER KARTOU", "creditcard withdrawal", trans$operation)
trans$operation <- ifelse(trans$operation == "VKLAD", "credit in cash", trans$operation)
trans$operation <- ifelse(trans$operation == "PREVOD Z UCTU", "collection from another bank", trans$operation)
trans$operation <- ifelse(trans$operation == "VYBER", "withdrawal in cash", trans$operation)
trans$operation <- ifelse(trans$operation == "PREVOD NA UCET", "remittance to another bank", trans$operation)

# K_symbol Werte verständlich machen
trans$k_symbol <- ifelse(trans$k_symbol == "POJISTNE", "insurrance payment", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "SLUZBY", "paymant for statement", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "UROK", "interest credited", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "SANKC. UROK", "sanction interest if negative balance", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "SIPO", "household", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "DUCHOD", "oldage pension", trans$k_symbol)
trans$k_symbol <- ifelse(trans$k_symbol == "UVER", "loan payment", trans$k_symbol)

# Der <int> wird in ein Datum umgewandelt.
trans$year <- ifelse(as.numeric(substring(trans$date, 1, 2)) < 10,
                        paste0("20", substring(trans$date, 1, 2)),
                        paste0("19", substring(trans$date, 1, 2)))

# Datum umwandeln
trans$date <- as.Date(paste0(trans$year, substring(trans$date, 3)), format = "%Y%m%d")
