...
Die Daten werden nun so angepasst, das sie auf Deutsch sind und die Types entsprechend geändert werden.
...
Zuerst werden wieder die Types angepasst und nschliessend wird die Spalte "issued" bearbeitet. Bei dieser sollen die ersten sechs Zeichen als Datum gewertet werden. Die 00:00:00 benötigen wir dabei nicht mehr, da diese bei allen Daten hintendran stehen.
...
Der loan Datensatz ist für uns einer der Interessantesten Datensätze. In ihm sind sämtliche vergebene Kredite abgelegt.
...
Nein gibt es nicht, jeder Account hat über die Zeit der Datenerhebung maximal einen Kredit bezogen.
...
![](Key Value Datensätze.PNG)

## ohne monatliche Saldos
...
transactions_accounts_with_loan %>%
  ...
  labs(title = "Amount zu Differenz für Status OK")
...
```{r}
set.seed(123)
splitIndex_13 <- createDataPartition(df_complete_loan_13_month$get_loan, p = .8, list = FALSE)
train_set_13 <- df_complete_loan_13_month[splitIndex_13, ]
test_set_13 <- df_complete_loan_13_month[-splitIndex_13, ]

model_13 <- randomForest(get_loan ~ . - account_id, data = train_set_13, method = "rf")

predictions_13 <- predict(model_13, test_set_13)

confusionMatrix(predictions_13, test_set_13$get_loan, mode = "everything", positive = "1")
```

## Predictions Kreditkandidaten

Erstellen eines Dataframes für alle ohne Loan in den letzten 13 Monaten
...
# Selecting balance columns (excluding account_id)
# balance_cols_df_no_loan_today <- setdiff(names(df_no_loan_today), "account_id")

# Applying Z-score normalization
# df_no_loan_today[balance_cols_df_no_loan_today] <- scale(df_no_loan_today[balance_cols_df_no_loan_today])
```

```{r}
# Predict using the Random Forest model on the new data
predictions_no_loan_today <- predict(model_13, df_no_loan_today)

# If you need to add these predictions as a new column to your df_no_loan_today dataframe:
df_no_loan_today$predictions <- predictions_no_loan_today

# View the first few rows of the dataframe to see the predictions
head(df_no_loan_today)

```

```{r}
# count predictions
table(predictions_no_loan_today)
varImpPlot(model_13)
```

```{r}
ids_predicted_yes <- df_no_loan_today %>%
  filter(predictions == 1) %>%
  select(account_id)

ids_predicted_no <- df_no_loan_today %>%
  filter(predictions == 0) %>%
  select(account_id)

transactions_accounts_no_loan <- trans %>%
  filter(account_id %in% df_no_loan_today$account_id) %>%
...
transactions_accounts_no_loan %>%
  ...
  summarize(gemitteltes_saldo = mean(saldo), .groups = "drop") %>%
  ggplot(aes(x = date, y = gemitteltes_saldo, group = predictions, color = predictions)) +
  geom_line() +
  geom_smooth(method = "loess", se = TRUE, aes(fill = predictions)) +
  ggtitle("Mittelwert des Saldos im letzten Jahr vor Kreditvergabe")
...
transactions_accounts_no_loan %>%
  ...
  geom_line() +
  geom_smooth(method = "loess", se = TRUE, aes(fill = predictions)) +
  ggtitle("Mittelwert der Einnahmen im letzten Jahr vor Kreditvergabe")
...
transactions_accounts_no_loan %>%
  ...
  geom_line() +
  geom_smooth(method = "loess", se = TRUE, aes(fill = predictions)) +
  ggtitle("Mittelwert der Ausgaben im letzten Jahr vor Kreditvergabe")
```
...
transactions_accounts_no_loan_for_training <- trans %>%
  filter(account_id %in% df_no_loan_today$account_id) %>%
  filter(date >= as.Date("1997-01-01") & date <= as.Date("1998-01-01")) %>%
  group_by(account_id, date) %>%
  summarise(
    einnahmen = sum(amount[type == "credit"]),
    ausgaben = sum(amount[type == "withdrawal in cash" | type == "withdrawal"]), .groups = "drop"
...
confusionMatrix(predictions_13, test_set_13$loan, mode = "everything", positive = "1")
```
...
# Predict using the Random Forest model on the new data
predictions_no_loan_today <- predict(model_13, df_grouped_metrics_no_loan_today)

# If you need to add these predictions as a new column to your df_grouped_metrics_no_loan_today dataframe:
df_grouped_metrics_no_loan_today$predictions <- predictions_no_loan_today

# View the first few rows of the dataframe to see the predictions
head(df_grouped_metrics_no_loan_today)
```
...
```{r}
# for training
df_grouped_metrics_plus <- df_grouped_metrics %>%
  left_join(client_analytical_record_no_district, by = "account_id") %>%
  mutate(
    has_user = ifelse(is.na(user_birthday), 0, 1),
    owner_age = 1999 - year.x,
  ...
  pivot_wider(names_from = card_type, values_from = card_type,
              values_fill = list(card_type = 0),
              values_fn = list(card_type = length)) %>%
  select(-user_birthday, -card_id, -user_disp_id, -user_client_id, -owner_client_id, -owner_birthday, -year.x, -year.y, -user_district_id, -user_sex, -card_issued, -opening_date)

# for prediction
df_grouped_metrics_no_loan_today_plus <- df_grouped_metrics_no_loan_today %>%
  left_join(client_analytical_record_no_district, by = "account_id") %>%
  mutate(
    has_user = ifelse(is.na(user_birthday), 0, 1),
    owner_age = 1999 - year.x,
...
glimpse(df_grouped_metrics_plus)
```

```{r}
set.seed(123)
splitIndex_13 <- createDataPartition(df_grouped_metrics_plus$loan, p = .8, list = FALSE)
train_set_13 <- df_grouped_metrics_plus[splitIndex_13, ]
test_set_13 <- df_grouped_metrics_plus[-splitIndex_13, ]
...
# Predict using the Random Forest model on the new data
predictions_no_loan_today <- predict(model_13, df_grouped_metrics_no_loan_today_plus)

# If you need to add these predictions as a new column to your df_grouped_metrics_no_loan_today_plus dataframe:
df_grouped_metrics_no_loan_today_plus$predictions <- predictions_no_loan_today

# View the first few rows of the dataframe to see the predictions
head(df_grouped_metrics_no_loan_today_plus)
varImpPlot(model_13)
```

```{r}

# You might want to scale/normalize your data before clustering
# df_grouped_metrics_plus <- scale(df_grouped_metrics_plus)

# Perform k-means clustering
set.seed(123) # Setting a seed for reproducibility
km_res <- kmeans(df_grouped_metrics_plus, centers = 3, nstart = 525)

# Visualize the clusters
fviz_cluster(km_res, data = df_grouped_metrics_plus)
