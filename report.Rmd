...
df_grouped_metrics_plus <- df_grouped_metrics %>%
  ...
  pivot_wider(names_from = card_type, values_from = card_type,
              values_fill = list(card_type = 0),
              values_fn = list(card_type = length)) %>%
  select(-user_birthday, -card_id, -user_disp_id, -user_client_id, -owner_birthday, -year.x, -year.y, -user_district_id, -user_sex, -card_issued, -opening_date)
...
```{r}
#find nas
nas <- sum(is.na(df_grouped_metrics_plus))
nas
```
...
km_res <- kmeans(df_grouped_metrics_plus, centers = 3, nstart = 25)
