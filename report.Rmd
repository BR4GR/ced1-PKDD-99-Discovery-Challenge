...
client_analytical_record <- client_analytical_record %>%
  left_join(district, by = c("user_district_id" = "district_id"))
...
#trans_per_month <- trans %>%
 # mutate(year_month = floor_date(date, unit = "month")) %>%
  ...
  #summarise(einnahmen = sum(amount[type == "credit"]),
  #(ausgaben = sum(amount[type == "withdrawal in cash"]))
...
trans_per_month <- trans %>%
  ...
  summarise(
    ...
    ausgaben = sum(amount[type == "withdrawal in cash" | type == "withdrawal"])
  )
