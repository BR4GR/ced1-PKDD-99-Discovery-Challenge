...
In diesem Plot ist gut zu sehen, das die Anzahl der Transaktionen pro Monat im Verlauf der Jahre deutlich zugenommen hat.
...
Bei den Transaktionen wollen wir nicht jede Transaktion einzeln an das konsolidierte Dataframe anhängen, sondern wollen nur die monatlichen Aktivitäten dokumentieren. Dazu erstellen wir ein Dataframe das die Einnahmen und Ausgaben pro Monat und Account zusammenfasst.
...
Als nächstes ersellen wir ein Dataframe, welches die Transaktionen für alle Accounts mit Kredit für die 6 Monate vor Kreditaufnahme enthält.
Mit diesem Können wir dan das Verhalten der Kunden vor der Kreditaufnahme analysieren.
...
Für das Training des Random Forests erstellen wir ein Dataframe, welches die Kennwerte wie Durchschnittlicher Kontostand, Einnahmen Ausgaben, Die Standartabweichungen der Einnahmen, Ausgabenund Kontostand, das Wachstum des Kontostandes der letzten 6 monate vor Kreditbegin für kunden mir Kredit und für die ersten 6 Monate des letzten Jahres für Kunden ohne Kredit enthält.
...
Mit dem erstellten Dataframe tranieren wir ein Random forest Model. Dafür splitten wir die daten und verwenden 80% für das Training und 20% für das Testen. Die Metriken Accuracy, Kappa und F1 werden ausgegeben.
...
```
Anschliesend haben wir die wichtigsten Variablen des Models ausgegeben. Diese sind nach der MeanDecreaseGini sortiert. Dieser Wert gibt an, wie stark die Varibale die Gini Impurity reduziert. Je höher der Wert desto wichtiger ist die Variable für das Model.
Wir sehen das die Wichtigsten Variablen mit den erstellten Kennwerten des Verhaltens zu tun haben, und auch der District eine wichtige Rolle spielt.
```{r}
...
Nach dem gleichen Schema wie bei dem Dataframe für das Training erstellen wir eines für die Predictions, mit dem Unterschied das wir für die daten nur accounts ohne Kredit und ds verhalten der letzten 6 Monate verwenden. Somit können wir das aktuelle Verhalten der Kunden vergleichen.
...
Nun können wir die Predictions für die Kunden ohne Kredit erstellen. zusätzlich sortieren wir sie auch nach der Warscheinlichkeit das der Kunde einen Kredit aufnimmt. Somit können wir auch eine bestimmte Anzahl Kunden mit der höchsten Warscheinlichkeit für ein Angebot auswählen.
...
### Verhalten der Kunden über zeit
Um das Visuel darzustellen haben wir uns entschieden das Verhalten Der Kunden über die letzten 6 Monate zu vergleichen.
