...
car_one_hot <- car_one_hot %>%
  filter(!is.na(user_sex)) %>%
  pivot_wider(names_from = user_sex, values_from = user_sex,
              values_fill = list(user_sex = 0),
              values_fn = list(user_sex = length)) %>%
  rename(
    user_female = female,
    user_male = male
  ) 
...
# categorical_columns <- c("owner_sex", "user_sex", "loan_status")
# car_one_hot <- client_analytical_record_no_district %>%
#   mutate(across(all_of(categorical_columns), ~factor(., exclude = NULL))) %>%
#   pivot_longer(cols = all_of(categorical_columns), values_drop_na = TRUE) %>% # Dropping NA values directly
#   mutate(value = 1) %>%
